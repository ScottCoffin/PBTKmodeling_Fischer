...
# Permeability
P_app           = 1.38*10^-6             # apparent permeability of the chemical (passive membrane diffusion) (cm/s)

# Partition coefficients to tissue constituents
K_FABP          = 10^2.29                  # Fatty acid binding protein-water partition coefficient (L_w/L_FABP)
K_SA            = 10^3.61                  # Human serum albumin-water partition coefficient (L_w/L_SA)
K_Glob          = 10^2.03                  # Globulin-water partition coefficient (L_w/L_SP)
K_SP            = 10^0.74                  # Structural proteins-water partition coefficient (L_w/L_SP)
K_ML            = 10^2.79                  # Membrane lipid-water partition coefficient (L_w/L_ML)

# Membrane transporters (expressed as permeabilities)
P_OATP1B2       = 1.45*10^-6             # OATP1B3-mediated active transport membrane permeability (cm/s)
P_OATP2B1       = 8.58*10^-7             # OATP2B1-mediated active transport membrane permeability (cm/s)
P_NTCP          = 1.81*10^-7             # NTCP-mediated active transport membrane permeability (cm/s)
P_OAT1          = 3.04*10^-6             # OAT1-mediated active transport membrane permeability (cm/s)
P_OAT2          = 1.32*10^-8             # OAT2-mediated active transport membrane permeability (cm/s)
P_OAT3          = 3.30*10^-5             # OAT3-mediated active transport membrane permeability (cm/s)
...
parameters <- c("P_app", "K_ML", "K_SA", "K_FABP", "K_Glob", "K_SP", "V_bile", 
                "V_SA_blood", "V_SA_liver", "V_SA_kidneys", "V_SA_gut", "V_SA_rest",
                "V_ML_blood", "V_ML_liver", "V_ML_gut", "V_ML_kidneys", "V_ML_rest",
                "V_SP_blood", "V_SP_liver", "V_SP_kidneys", "V_SP_gut", "V_SP_rest",
                "V_Glob_blood", "V_FABP_liver", "V_FABP_kidneys", "V_FABP_gut")
...
for (param in parameters) {
  ...
  if (param == "K_ML" || param == "K_SA" || param == "K_FABP" || param == "K_Glob" || param == "K_SP" || param == "V_SA_blood" || param == "V_SA_liver" || param == "V_SA_kidneys" || param == "V_SA_gut" || param == "V_SA_rest" || param == "V_ML_blood" || param == "V_ML_liver" || param == "V_ML_gut" || param == "V_ML_kidneys" || param == "V_ML_rest" || param == "V_SP_blood" || param == "V_SP_liver" || param == "V_SP_kidneys" || param == "V_SP_gut" || param == "V_SP_rest" || param == "V_Glob_blood" || param == "V_FABP_liver" || param == "V_FABP_kidneys" || param == "V_FABP_gut") {
    K_blood       = V_water_blood / V_blood + V_SA_blood / V_blood * K_SA + V_Glob_blood / V_blood * K_Glob + V_SP_blood / V_blood * K_SP + V_ML_blood / V_blood * K_ML  # Blood-water partition coefficient (cm?/cm?), equal for all blood components
    K_liver       = V_water_liver / V_liver_tissue + V_FABP_liver / V_liver_tissue * K_FABP + V_SA_liver / V_liver_tissue * K_SA + V_SP_liver / V_liver_tissue * K_SP + V_ML_liver / V_liver_tissue * K_ML # Liver-water partition coefficient (cm?/cm?)
    K_bile        = V_water_bile / V_bile + V_SA_bile / V_bile * K_SA + V_ML_bile / V_bile * K_ML # Bile-water partition coefficient (cm?/cm?)  
    K_bile_liver  = K_bile / K_liver
